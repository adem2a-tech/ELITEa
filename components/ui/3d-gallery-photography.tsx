...
function GalleryScene({
  ...
  const handleWheel = useCallback(
    (event: WheelEvent) => {
      ...
      setScrollVelocity((prev) => prev + event.deltaY * 0.03 * speed)
      setAutoPlay(false)
  ...
  useEffect(() => {
    const interval = setInterval(() => {
      if (Date.now() - lastInteraction.current > autoPlayDelayMs) {
        setAutoPlay(true)
      }
    }, 500)
  ...
  useFrame((state, delta) => {
    ...
    setScrollVelocity((prev) => prev * 0.96)
...
function FallbackGallery({ images }: { images: ImageItem[] }) {
  ...
  return (
    <div className="flex h-full flex-col items-center justify-center bg-secondary p-4">
      <p className="mb-4 text-sm text-muted-foreground">
        WebGL non supporté. Affichage des images :
      </p>
